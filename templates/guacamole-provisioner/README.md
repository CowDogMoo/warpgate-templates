...
Optimized sidecar container for dynamically provisioning Apache Guacamole
connections from Kubernetes secrets.

## Features

### Performance Optimizations

1. **Pre-installed Dependencies**: All Python packages are installed at build
   time, reducing startup from ~40-60s to ~5-10s
2. **Connection Pooling**: Reuses database connections instead of creating
   new ones every cycle
3. **Batch Operations**: Uses `psycopg2.extras.execute_batch` for efficient
   bulk inserts
4. **State Caching**: Computes file hashes to detect changes and skip
   unnecessary updates
5. **Smart Updates**: Compares database state with new config to avoid
   redundant writes
...
| ---------- | --------- | ------------- |
| `CONNECTIONS_DIR` | `/connections` | Directory with connection YAML files |
